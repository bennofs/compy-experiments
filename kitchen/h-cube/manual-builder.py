...
def build(runtime: Runtime, work_dir, metadata, args):
    ...
    out_dir = f"{args.out}/{package_id}-{commit}"
    ...
    if not args.build_only:
        ...
        runtime.spawn(runtime.config_container(rootfs, [
            "apt-get", "-y", "--force-yes", "install", "devscripts", "autoconf", "automake", "autotools-dev", "cmake", "libtool", "gnulib"
        ]))
    ...
    shutil.rmtree(build_dir + "/src")
    repo = pygit2.Repository(args.repos_dir + "/" + metadata['repo_name'])
    tree = repo[commit].tree
    repo.checkout_tree(tree, directory=build_dir + "/src", strategy=pygit2.GIT_CHECKOUT_FORCE|pygit2.GIT_CHECKOUT_RECREATE_MISSING)

    shutil.copy(args.buildscripts_dir + "/" + commit, rootfs + "/script")
    ...
    c = runtime.config_container(rootfs, strace_cmd + ["/bin/bash", "/script"], workdir="/build/src/" + metadata['workdir'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--buildscripts-dir", help="Directory containing build scripts")
