...
cc_compile_regex = re.compile(rb"^.*-?g?cc-?[0-9.]*$|^.*-?clang-?[0-9.]*$")
cpp_compile_regex = re.compile(rb"^.*-?[gc]\+\+-?[0-9.]*$|^.*-?clang\+\+-?[0-9.]*$")
file_regex = re.compile(rb"^.+\.c$|^.+\.cc$|^.+\.cpp$|^.+\.cxx$|^.+\.s$", re.IGNORECASE)
compiler_wrappers = {"ccache", "icecc", "sccache"}

# Leverage `make --print-directory` option
make_enter_dir = re.compile(rb"^\s*make\[\d+\]: Entering directory [`\'\"](?P<dir>.*)[`\'\"]\s*$")
make_leave_dir = re.compile(rb"^\s*make\[\d+\]: Leaving directory .*$")

# We want to skip such lines from configure to avoid spurious MAKE expansion errors.
checking_make = re.compile(rb"^checking whether .* sets \$\(\w+\)\.\.\. (yes|no)$")
...
class MakeLogParser:
    ...
    def __init__(self, working_dir, rootfs: Path):
        self.rootfs = rootfs
