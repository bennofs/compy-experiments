...
def main(args):
    ...
    for i, (train_names_idx, test_names_idx) in enumerate(split):
        ...
        test_samples = np.array([s for s in samples if s['info']['name'] in test_names])

        rng = np.random.default_rng(seed=0)
        rng.shuffle(train_samples)

        train_y_true = sum(1 for s in train_samples if s['y'])
        test_y_true = sum(1 for s in test_samples if s['y'])
        print(f"true rate in train ({len(train_samples)}): {train_y_true / len(train_samples)}")
        print(f"true rate in test ({len(test_samples)}: {test_y_true / len(test_samples)}")
